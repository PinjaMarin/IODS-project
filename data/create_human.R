#Pinja Marin
#creating the human data through combining two datasets, and making some modifications to the data
...
# I am using the data I created last week but the combined data is also available here: http://s3.amazonaws.com/assets.datacamp.com/production/course_2218/datasets/human1.txt
...
human2 <- human2[, 2:20]
dim(human2) #195,19
str(human2) # there are 19 variables. Except for the country variable, all variables give numerical information about the coutries (and areas) on matters such as equality and development. The variable GNI (Gross National Income per capita) appears to be in a character form although it is supposed to be a numerical value. Besides GNI, some example variables are "life_exp" = Life expectancy at birth, and "Edu2_f" = Proportion of females with at least secondary education. 

human2$country #this variable mainly refers to different countries but there also some larger areas such as "Europe and Central Asia" and even "World". 
#More information about the dataset can be found earlier in this script, in the two links from last week.
...
keep <- c("country", "edu2_ftom", "labour_ftom", "Edu_exp", "life_exp", "GNI", "mat_mor", "Adol_birth", "parl_f")
...
#
# first, printing out the data along with a completeness indicator as the last column (it shows whether there are any missing values per row)
...
human_f$country #it seems that the last 7 observations are not countries but regions. Let's remove them. 
...
#Save the human data in your data folder including the row names. Let's not overwrite the old ‘human’ data in case there are any issues and instead keep this data as 'humanf'.
